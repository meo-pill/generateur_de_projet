...
if ( test -e "$emplacementProjet.tar.gz" ) ; then
	if ( test -d "$emplacementProjet" ) ; then
		if ( test $(ls -a $emplacementProjet | grep -v "^\./$" | grep -v "^\.\./$" | wc -l) -eq 0 ) ; then
			...
			$questionneur :v "Continuer au risque d'avoir une incompatibilité entre les deux ?" "abandonner" ; retour=$?
			if ( test $? -eq 0 ) ; then
