	...
	case "$1" in
		...
		*) #Test du paramètre optionnel <action> :
			if ( test -z "$action" ) then
				...
				echo "L'utilité de l'objet à déjà était renseigné ($action)."
				echo -e "$Usage\n$Detail_action"
				exit 2
			fi
	esac
...
F_Src="$dossierProjet/$Src/$nomF.$ext_C"
F_Inc="$dossierProjet/$Inc/$nomF.$ext_H"
F_Test="$dossierProjet/$Test/$preTest$nomF.$ext_C"

#	# Création des fichiers.
cp "$dossierProjet/$Mod/$Src.$ext_C" $F_Src
cp "$dossierProjet/$Mod/$Inc.$ext_H" $F_Inc
cp "$dossierProjet/$Mod/$Test.$ext_C" $F_Test
...
sed -i -e "s/~~NOMDEF~~/$nomDef/g" $F_Inc
...
clear
ls *
echo -e "\n\n\t\tLes fichier '$(basename $Src/$F_Src)', '$(basename $Inc/$F_Inc)' et '$(basename $Test/$F_Test)' ont était crée.\n\n"
echo -e "\n\tFIN DU SCRIPT '$0'."
exit 0
